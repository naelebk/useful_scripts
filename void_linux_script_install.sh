...
updateee() {
    echo -ne "${YELLOW}Update du système en cours..... ${NC}"
...
install_deb() {
    if ! command -v "alien" > /dev/null 2>&1; then
        echo -ne "${YELLOW}Installation de Alien pour conversion de .$extension en .$package_manager.....${NC} "
...
if ! command -v xbps-install >/dev/null 2>&1; then
    echo -e "${RED}Erreur : le système doit être basé sur Void Linux pour exécuter le script.${NC}" 2>&1
    exit 3
fi
if [[ "$(whoami)" != "root" ]]; then
...
echo -ne "${YELLOW}Nettoyage du système ($package_manager-remove) : ${NC}"
